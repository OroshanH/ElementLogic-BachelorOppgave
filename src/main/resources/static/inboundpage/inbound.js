...
function formaterData(produkter){
    var ut = "<table class='table table-light table-hover font center-table'>" +
        "<tr>" +
        "<th scope='col' class=thLabel>ProduktID</th><th scope='col' class=thLabel>Navn</th><th scope='col' class=thLabel>Beskrivelse</th><th scope='col' class=thLabel>Antall</th><th scope='col' class=thLabel>Send Produkt</th>" +
        "</tr>";
    for(let i in produkter ){
        ut+="<tr><td class='th'>"+produkter[i].produktid+"</td><td class='th'>"+produkter[i].navn+"</td><td class='thB'>"+produkter[i].beskrivelse+"</td><td class='thInp'>" +  "<input type='number' min='0' id='quantity" + i + "'>" + "</td>" + "<td>" + "<button id='myButton' onclick='sendProdukt(" + produkter[i].produktid +  ", " + i + ")' class='btnSend'>Send</button>" + "</td> </tr>"
    }
    $("#produktene").html(ut);
}
